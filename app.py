...
║  Flask API يربط الموقع بـ bot.db المشترك                                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
from flask import Flask, request, jsonify, send_from_directory, abort
import psycopg2, psycopg2.extras, os, hashlib, base64, json, time, logging
...
_ENC_KEY_RAW = os.environ.get("ENC_KEY")
if not _ENC_KEY_RAW:
    raise RuntimeError(
        "❌ ENC_KEY غير موجود في متغيرات البيئة!\n"
        "أضف ENC_KEY في إعدادات Railway — يجب أن يكون نفس مفتاح البوت."
    )
...
def verify():
    ...
    # ── أعد البيانات ──
    issue_date = (report["created_at"] or "")[:10]
    try:
        from datetime import datetime as dt
        d = dt.strptime(issue_date, "%Y-%m-%d")
    ...
    result = {
        ...
        "national_id":     national_id[:3] + "****" + national_id[-3:],  # إخفاء جزء من الرقم
...
def add_website():
    ...
    try:
        with _dbctx() as c:
            c.execute("INSERT INTO websites(name,url,slot_number,max_reports) VALUES(%s,%s,%s,%s) ON CONFLICT(slot_number) DO UPDATE SET name=EXCLUDED.name, url=EXCLUDED.url, max_reports=EXCLUDED.max_reports",
                      (name, url, slot, maxr))
            c.commit()
