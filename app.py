...
def seed_test_data():
    """إدخال بيانات تجريبية مؤقتة — يُحذف لاحقاً"""
    try:
        with _dbctx() as c:
            c.execute("""
                INSERT INTO reports
                    (report_number, patient_id, patient_name, nationality, employer,
            ...
            """, (
                ...
                'أحمد العتيبي',
                'جراحة العظام',
                'مستشفى د. سليمان الحبيب التخصصي',
                'official',
                '{"issue_date": "19-04-2026", "birth_date": ""}'
            ))
            c.commit()
        logger.info("✅ البيانات التجريبية أُدخلت بنجاح")
    except Exception as e:
        logger.error(f"❌ خطأ في إدخال البيانات التجريبية: {e}")
